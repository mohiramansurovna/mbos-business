...
export default function Header({
	...
	return (
		<>
			<div className='fixed md:hidden backdrop-blur-3xl top-0 left-0 w-screen h-16 z-40 flex justify-between items-center py-5 px-3'>
			...
			<header
				...
				className={`transition-all duration-300
                    ...
                    fixed z-50 w-4/5 md:w-full backdrop-blur-xl md:backdrop-blur-sm flex flex-col md:flex-row justify-start pt-20 md:pt-0 md:justify-around items-center h-screen md:h-25 overflow-hidden text-white`}
			>
				<Link onClick={handleMenu} href='#home'>
					<Image
						src='/logo.svg'
				...
				<nav className='flex flex-col text-[18px] font-light  items-center md:items-start md:flex-row gap-3'>
					<Link
						onClick={handleMenu}
						href='#home'
						className='hover:text-gray-400 py-1.5 px-3 rounded-sm active:bg-white active:text-primary-blue text-mm'
					>
						{t.header.nav.main}
					</Link>
					<Link
				...
				<div className='flex gap-4 md:flex-row flex-col items-center'>
					...
					<LocaleSwitcher />
					<button className='px-4 py-2.5 border-sm bg-mbosColor hover:bg-mbosColor/80 rounded-md text-white font-semibold text-mm'>
						{t.header.login}
					</button>
				</div>
			</header>
